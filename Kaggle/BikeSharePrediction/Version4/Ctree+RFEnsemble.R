...
treeFit = ctree(count ~ season + holiday + workingday + weather + temp + atemp + humidity + 
                        hour + Sunday + daySlot, data = bikeTrain)
treePredictions = predict(treeFit, newdata = bikeTest)
head(treePredictions)

countPred = round(treePredictions, 0)
countPred[countPred < 0]

submission = data.frame(datetime = bikeTest$datetime, count = countPred)
write.csv(submission, 'CTree.csv', row.names = FALSE)

library(randomForest)

optimumMTry = tuneRF(bikeTrain[, c('season', 'holiday', 'workingday', 'weather', 'temp', 
                'atemp', 'humidity', 'Sunday','daySlot')], bikeTrain[, 'count'])

rfModel = randomForest(count ~ season + holiday + workingday + weather + temp + atemp +
                               humidity + Sunday + daySlot, 
                       data = bikeTrain, mtry = 3, nodesize = 25, ntree = 500)
...
submission = data.frame(datetime = bikeTest$datetime, count = countPred)
write.csv(submission, 'CTreeRFEnsemble.csv', row.names = FALSE)