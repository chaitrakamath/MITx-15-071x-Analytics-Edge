...
optimumMTry = tuneRF(bikeTrain[, c('season', 'holiday', 'workingday', 'weather', 'temp', 
                                   'atemp', 'humidity', 'windspeed', 'Sunday','daySlot')],
                     bikeTrain[, 'count'])


rfModel = randomForest(count ~ season + holiday + workingday + weather + temp + atemp +
                               humidity + windspeed  + Sunday + daySlot, 
                       data = bikeTrain, mtry = 3, nodesize = 25, ntree = 200)
...
tr = train(count ~ season + holiday + workingday + weather + temp + atemp +
                   humidity + windspeed + Sunday + daySlot, data = bikeTrain, 
           method = "rpart", trControl = tr.control, tuneGrid = cp.grid)


treePredictions = predict(tr, newdata = bikeTest)

predictions = (rfPredictions + treePredictions) / 2
countPred = round(predictions, 0)
countPred[countPred < 0]

submission = data.frame(datetime = bikeTest$datetime, count = countPred)
write.csv(submission, file = 'RF+CARTEnsembleV3.csv', row.names = FALSE)
