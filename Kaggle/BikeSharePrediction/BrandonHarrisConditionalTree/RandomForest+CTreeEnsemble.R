...
optimumMTry = tuneRF(train_factor[, c('season', 'holiday', 'workingday', 'weather', 'temp',  
                        'atemp', 'humidity','hour', 'daypart' , 'sunday')], train_factor[, 'count'])


rfModel = randomForest(formula,data = train_factor, mtry = 3, nodesize = 25, ntree = 500)

rfPredictions = predict(rfModel, newdata = test_factor)

rfCount = round(rfPredictions, 0)
rfCount[rfCount < 0]

submission = data.frame(datetime = test_factor$datetime, count = rfCount)
write.csv(submission, 'RandomForestFinal.csv', row.names = FALSE)


predictions = (rfPredictions + predict.ctree) / 2
countPred = round(predictions, 0)

submission = data.frame(datetime = test_factor$datetime, count = countPred)
write.csv(submission, 'RF+CTreeEnsembleV2.csv', row.names = FALSE)
